...
function localserver.GetStore(name, db_name)
    if (db_name == nil) then
...
function localserver.FlushAll()
    if (localserver.WebCacheDB and localserver.WebCacheDB.FlushAll) then
        localserver.WebCacheDB.FlushAll()
    end
end
