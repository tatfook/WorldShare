...
function localserver:Open(server_id)
	...
	self.security_origin_ = System.localserver.SecurityOrigin:new(server.security_origin_url);
end
...
function localserver:CreateOrOpen(security_origin, name, required_cookie) 
	...
	if(type(security_origin) == "string") then
		security_origin = System.localserver.SecurityOrigin:new(security_origin);
	end
