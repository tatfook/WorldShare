...
-- default policy
ResourceStore.Cache_policy = localserver.CachePolicy:new("access plus 1 day")
...
function ResourceStore:Open(server_id)
	if (not localserver.Open(self, server_id)) then
		return
	end
	self.is_initialized_ = true
	local version = self:GetVersion(WebCacheDB.VersionReadyState.VERSION_CURRENT)
...
function ResourceStore:Delete(url)
	if (not url or not self.is_initialized_) then
		return
	end
...
function ResourceStore:Rename(orig_url, new_url)
	if (not orig_url or not new_url or not self.is_initialized_) then
		return
	end
...
function ResourceStore:Copy(src_url, dst_url)
	if (not src_url or not dst_url or not self.is_initialized_) then
		return
	end
