...
function FileDownloader:Start(src, dest, callbackFunc, cachePolicy)
    ...
    if(self.text ~= "official_texture_package") then
        BroadcastHelper.PushLabel({id="noWrap", label = format(L"%s: 正在下载中,请耐心等待", self.text), max_duration=20000, color = "255 0 0", scaling=1.1, bold=true, shadow=true,});
    end
    local res = ls:GetFile(System.localserver.CachePolicy:new(cachePolicy or "access plus 5 mins"),
        ...
        function (msg, url)
            ...
            if(text and self.text ~= "official_texture_package") then
                BroadcastHelper.PushLabel({id="noWrap", label = format(L"文件%s: %s", self.text, text), max_duration=10000, color = "255 0 0", scaling=1.1, bold=true, shadow=true,});
            end	
        end
    );
    if(not res) then
